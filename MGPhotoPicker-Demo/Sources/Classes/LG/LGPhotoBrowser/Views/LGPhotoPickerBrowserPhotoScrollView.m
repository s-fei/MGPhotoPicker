...
- (void)setPhoto:(LGPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoPath.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            [_photoImageView sd_setImageWithPreviousCachedImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
- (void)layoutPhotoLoadOriginButton
{
    ...
    if ([_photo respondsToSelector:@selector(loadOriginButton)])
    {
        ...
        if (_loadOriginButton)
        {
            ...
            [_loadOriginButton setFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 148)/2, [UIScreen mainScreen].bounds.size.height - 42, 148, 32)];
